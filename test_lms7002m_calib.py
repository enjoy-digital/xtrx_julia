...
def lms7002m_delay_tx_calib_test(port):
    bus = RemoteClient(port=port)
    bus.open()
...
def lms7002m_delay_rx_calib_test(port):
    bus = RemoteClient(port=port)
    bus.open()
...
def main():
    ...
    parser.add_argument("--tx-calib",     action="store_true", help="Run TX calibration test.")
    parser.add_argument("--rx-calib",     action="store_true", help="Run RX calibration test.")
    ...
    if args.tx_calib:
        lms7002m_delay_tx_calib_test(port=port)

    if args.rx_calib:
        lms7002m_delay_rx_calib_test(port=port)
